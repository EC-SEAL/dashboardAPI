...
from . import json_variables as JsonVariables
...
def JsonConstructor(_UUID='', _address='', _msToken='', _bindingMethod='', _identities='', _ERROR=JsonVariables.Error.NO_ERROR):
    ...
    try:
        ...
        # List of params as dict
        parameters = list(locals().keys())[:-1]

        response_content = ''

        for index, param in enumerate(parameters):
            if locals().get(param,'') != '':                
                if index > 0 and locals().get(parameters[index - 1], '') != '':
                    response_content = response_content + ','

                if param[1:] == 'identities':
                    param_value = locals().get(param)
                else:
                    param_value = '"' + locals().get(param) + '"'           

                response_content += JsonVariables.Response.element_string.format(element_name=param[1:],response_value=param_value)

        response = JsonVariables.Response.response_base.format(response_description="test",response_elements=response_content)

        return json.loads(response)

    except:
        _ERROR = '"' + _ERROR + '"'
        response_content = JsonVariables.Response.element_string.format(element_name="Error",response_value=_ERROR)
        response = JsonVariables.Response.response_base.format(response_description="Error",response_elements=response_content)
        return json.loads(response)
